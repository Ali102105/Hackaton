...
const THEMES = [
  ...
  { id: 'synthwave',label: 'Synth',    colors: ['#0D0520','#FF00E6','#FFD000'] },
  { id: 'matrix',   label: 'Matrix',   colors: ['#000800','#00FF41','#004400'] },
  { id: 'light',    label: 'Light',    colors: ['#F5F5FA','#7C3AED','#0284C7'] },
  { id: 'minimal',  label: 'Minimal',  colors: ['#0A0A0A','#FFFFFF','#888888'] },
];
...
function buildGrid(filter = 'all') {
  ...
  document.getElementById('filter-count').textContent = `${visible.length} game${visible.length !== 1 ? 's' : ''}`;
...
function startPreviewAnimations() {
  document.querySelectorAll('.preview-canvas').forEach(canvas => {
    ...
    function draw() {
      ...
      if (id === 'snake') {
        ...
        blocks.forEach(([bx, by]) => {
          ...
          const y = (canvas.height/2 - 20 + by*20 + frame*0.4) % canvas.height;
          ctx.fillRect(x, y, 18, 18);
        });
      } else if (id === 'breakout') {
        // bouncing ball
        const bx = canvas.width/2 + Math.sin(frame*0.04) * canvas.width*0.3;
        const by = canvas.height/2 + Math.sin(frame*0.06) * canvas.height*0.3;
        ctx.globalAlpha = 0.8;
        ctx.beginPath();
        ...
        // two paddles + ball
        ctx.globalAlpha = 0.5;
        ctx.fillRect(10, canvas.height/2 + Math.sin(frame*0.05)*20, 8, 40);
        ctx.fillRect(canvas.width-18, canvas.height/2 + Math.cos(frame*0.05)*20, 8, 40);
        ctx.beginPath();
        ctx.arc(canvas.width/2 + Math.sin(frame*0.07)*50, canvas.height/2 + Math.cos(frame*0.09)*30, 5, 0, Math.PI*2);
        ctx.fill();
      } else if (id === 'flappy') {
        // bird
        ctx.globalAlpha = 0.7;
    ...
    if (previewAnimations.has(id)) cancelAnimationFrame(previewAnimations.get(id));
    draw();
  });
}
...
function updateBestStat() {
  ...
  document.getElementById('best-score-stat').textContent = best > 0 ? best : '—';
}
...
function startTetris() {
  const canvas = document.getElementById('game-canvas');
  const ctx = canvas.getContext('2d');
  const W = 10, H = 20, S = Math.floor(canvas.width / W);
  canvas.height = H * S;
  ...
  function collide(ox = 0, oy = 0, shape = cur.shape) {
    return shape.some((row, r) => row.some((v, c) => {
      if (!v) return false;
      const nx = curX + c + ox, ny = curY + r + oy;
  ...
  function onKey(e) {
    if (['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key)) e.preventDefault();
    if (e.type === 'keydown') {
      if (e.key === 'ArrowLeft')  { if (!collide(-1)) curX--; draw(); }
      if (e.key === 'ArrowRight') { if (!collide(1))  curX++; draw(); }
      if (e.key === 'ArrowDown')  { if (!collide(0,1)) curY++; else place(); draw(); }
      if (e.key === 'ArrowUp') {
        const rotated = rotate(cur.shape);
        const old = cur.shape; cur.shape = rotated;
        if (collide()) { // wall kick
          if (!collide(-1)) curX--;
...
function startBreakout() {
  ...
  const COLORS = ['#FF006E','#FF4500','#FF8C00','#FFD700','#00FF88','#00F5FF','#A855F7','#FF69B4'];
  ...
  function draw() {
    ...
    bricks.forEach(b => {
      ...
      ctx.beginPath(); ctx.roundRect(b.x+1, b.y+1, b.w-2, b.h-2, 2); ctx.fill();
    ...
    pg.addColorStop(0, '#FF8C00'); pg.addColorStop(1, '#FFD700');
    ctx.fillStyle = pg;
    ...
    for (let i = 0; i < lives; i++) {
      ...
      ctx.beginPath(); ctx.arc(12 + i*22, 18, 6, 0, Math.PI*2); ctx.fill();
    }
    ctx.shadowBlur = 0;
  }
  ...
  draw();
  STATE.rafId = requestAnimationFrame(ts => { lastTime = ts; STATE.rafId = requestAnimationFrame(tick); });
}
...
function startPong() {
  ...
  function tick(ts) {
    ...
    // ai paddle
    if (bx + BALL_R > W - 15 - PW && bx + BALL_R < W - 15 && by > ay && by < ay + PH) {
      vx = -Math.abs(vx) * 1.04;
      vy = ((by - (ay + PH/2)) / (PH/2)) * 5;
      bx = W - 15 - PW - BALL_R; rally++;
    }
...
function startFlappy() {
  ...
  let pipes = [];
  let score = 0, frame = 0, pipeTimer = 0;
  ...
  function draw() {
    ...
    sky.addColorStop(0, '#0D0D00'); sky.addColorStop(1, '#1A1A00');
    ...
    ctx.beginPath(); ctx.ellipse(-4, -3, bird.r * 0.5, bird.r * 0.4, -0.3, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#000';
  ...
  function tick(ts) {
    ...
    pipeTimer += dt;
    if (pipeTimer > 80) { spawnPipe(); pipeTimer = 0; }
    ...
    // collision
    if (bird.y - bird.r < 0 || bird.y + bird.r > H) { onGameOver(score); return; }
    for (const p of pipes) {
      if (bird.x + bird.r > p.x && bird.x - bird.r < p.x + PIPE_W) {
        if (bird.y - bird.r < p.top || bird.y + bird.r > p.top + GAP) { onGameOver(score); return; }
      }
    }
...
function startAsteroids() {
  ...
  function explode(x, y, color) {
    for (let i = 0; i < 12; i++) {
      ...
      particles.push({ x, y, vx: Math.cos(a)*s, vy: Math.sin(a)*s, life: 35, color });
    }
  }
  ...
  function draw() {
    ...
    if (invincible <= 0 || Math.floor(invincible / 4) % 2 === 0) {
      ...
      if (keys['ArrowUp'] || keys['w']) {
        ...
        ctx.beginPath(); ctx.moveTo(-5, ship.r*0.3); ctx.lineTo(0, ship.r+8+Math.random()*6); ctx.lineTo(5, ship.r*0.3); ctx.stroke();
      }
      ctx.shadowBlur = 0;
      ctx.restore();
    }
    ...
    // lives
    for (let i = 0; i < lives; i++) {
      ctx.save(); ctx.translate(15 + i*22, 20); ctx.rotate(-Math.PI/2);
      ctx.strokeStyle = '#00F5FF'; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(0,-8); ctx.lineTo(-5,5); ctx.lineTo(0,2); ctx.lineTo(5,5); ctx.closePath(); ctx.stroke();
      ctx.restore();
    }
  ...
  function tick(ts) {
    ...
    asteroids.forEach(a => { a.x += a.vx*dt; a.y += a.vy*dt; a.angle += a.rot*dt; wrap(a); });
    ...
    for (let bi = bullets.length-1; bi >= 0; bi--) {
      for (let ai = asteroids.length-1; ai >= 0; ai--) {
        ...
        if (Math.hypot(b.x-a.x, b.y-a.y) < a.r) {
          ...
          if (a.r > 22) {
            for (let k = 0; k < 2; k++) {
              const newA = { x: a.x, y: a.y, vx: (Math.random()-0.5)*4, vy: (Math.random()-0.5)*4, r: a.r*0.55, angle: 0, rot: (Math.random()-0.5)*0.06, pts: [] };
              buildPoints(newA); asteroids.push(newA);
            }
          }
